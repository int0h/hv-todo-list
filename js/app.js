...
function normalizeNodeSet(hs, children) {
    ...
    if (children instanceof hyper_value_1.HyperValue) {
...
function targetRenderChildren(meta, children) {
    var hs = new hyper_value_1.scopes.ProxyScope();
...
exports.renderDebug = hyper_value_1.tracer.traceMethod(function (target) { return target.constructor.name; });
...
var HyperElm = /** @class */ (function (_super) {
    ...
    HyperElm.prototype.initAttrs = function (meta) {
        var _this = this;
        this.propHv = new hyper_value_1.HyperValue({});
...
registerGlobalProp('ref', function (_a) {
    var owner = _a.owner, value = _a.value;
    value(owner);
});
...
var Wrapper = /** @class */ (function (_super) {
    ...
    Wrapper.prototype.render = function () {
        return hv_jsx_1.jsx("div", null, router.content);
    };
    return Wrapper;
}(hv_jsx_1.Component));
hv_dom_1.renderIn(document.body, {}, hv_jsx_1.jsx(Wrapper, null));
...
var Router = /** @class */ (function (_super) {
    ...
    function Router(cfg, routes) {
        var _this = _super.call(this, cfg, routes) || this;
        _this.hs = new hyper_value_1.scopes.ObjectScope();
        router_core_1.handleHistory(_this);
        _this.onTransition(function (type, oldCW, newCW, handleHistory) {
            _this.inTransition = true;
            switch (type) {
                case 'routeChange':
                    _this.routeName.$ = newCW.routeName;
                    _this.routeData.$ = newCW.data;
                    break;
                case 'pathChange':
                case 'paramChange':
                    _this.routeData.$ = newCW.data;
    ...
    Router.prototype.renderRoot = function (routeName, routeData) {
        return hv_jsx_1.jsx(this.routes[routeName].meta.component, { routeName: routeName, routeData: routeData });
    };
    Router.prototype.initWithRoute = function (route, data) {
        var _this = this;
        _super.prototype.initWithRoute.call(this, route, data);
        this.cvhv = new hyper_value_1.HyperValue(this.currentView);
        this.routeName = this.hs.prop(this.cvhv, 'routeName');
        this.routeData = this.hs.prop(this.cvhv, 'data');
        this.hs.watch(this.cvhv, function (newCV) {
            if (!_this.inTransition) {
                _this.go(newCV.routeName, newCV.data);
            }
        });
        this.content = this.hs.auto(function () { return _this.renderRoot(_this.routeName.$, _this.routeData); });
        history.replaceState(this.currentView.state, '', location.href);
    };
    return Router;
}(router_core_1.RouterCore));
...
var RouterCore = /** @class */ (function () {
    function RouterCore(cfg, routes) {
        ...
        this.routes = routes;
        for (var name_1 in routes) {
            routes[name_1].name = name_1;
        }
        this.cfg = cfg;
    }
    RouterCore.prototype.resolveRoute = function (route) {
        var found = typeof route === 'string'
            ? this.routes[route]
            : route;
        if (!found) {
    ...
    RouterCore.prototype.match = function (url) {
        for (var routeName in this.routes) {
            var data = this.routes[routeName].parse(url);
            if (data) {
    ...
    RouterCore.prototype.build = function (route, data) {
        return this.resolveRoute(route).stringify(data);
    };
    RouterCore.prototype.initWithRoute = function (route, data) {
        this.go(route, data, true);
    };
    RouterCore.prototype.init = function () {
        var url = this.cfg.hashPrefix
            ? location.hash.slice(1)
            : location.href.slice(location.origin.length);
        var currentView = this.match(url);
        if (!currentView) {
    ...
    RouterCore.prototype.go = function (route, data, silent, handleHistory) {
        var _this = this;
        if (handleHistory === void 0) { handleHistory = true; }
        var resolved = this.resolveRoute(route);
        var newCW = {
        ...
        if (this.currentView.route === resolved) {
            var pathChanged = resolved.pathParams.some(function (paramName) {
                return _this.currentView.data[paramName] !== data[paramName];
            });
        ...
        this.transitionHandlers.forEach(function (handler) { return handler(type, _this.currentView, newCW, handleHistory); });
        this.currentView = newCW;
    };
    RouterCore.prototype.changeParams = function (dataDiff) {
        var newData = Object.assign({}, this.currentView.data);
        for (var name_2 in dataDiff) {
            newData[name_2] = dataDiff[name_2];
        }
        this.go(this.currentView.route, newData);
    };
    RouterCore.prototype.onTransition = function (handler) {
...
exports.route = route;
function parseParam(param, code) {
...
var Route = /** @class */ (function () {
    function Route(pattern, cfg) {
        ...
        this.pattern.forEach(function (item) {
            if (typeof item !== 'string') {
                _this.pathParams.push(item.$);
            }
        });
        this.params = {};
        if (cfg.params) {
            for (var name_1 in cfg.params) {
                var param = cfg.params[name_1];
                if (param instanceof RegExp) {
                    param = { re: param };
                }
                this.params[name_1] = param;
            }
        }
    }
    Route.prototype.stringify = function (data) {
        var usedKeys = [];
        var path = '/' + this.pattern.map(function (part) {
        ...
        for (var name_2 in data) {
            if (usedKeys.indexOf(name_2) === -1) {
                restParams[name_2] = data[name_2];
            }
        }
        return path + '?' + utils_1.stringifyQuery(restParams);
    };
    Route.prototype.parsePath = function (path) {
        var data = {};
        path = utils_1.trimSlashes(path);
        var parts = path.split('/');
        for (var i = 0; i < this.pattern.length; i++) {
            var urlPart = parts[i];
            var patternPart = this.pattern[i];
            if (!urlPart) {
                return null;
            }
            ...
            var paramName = patternPart.$;
            var param = this.params[paramName];
            var parsed = parseParam(param, urlPart);
            if (parsed === null) {
                return null;
            }
    ...
    Route.prototype.parse = function (url) {
        var _a = utils_1.parseUrl(url), path = _a.path, query = _a.query;
        var pathData = this.parsePath(path);
        if (!pathData) {
            return null;
        }
        return Object.assign({}, pathData, query);
    };
    Route.prototype.getState = function (data) {
        return { data: data, routeName: this.name };
    };
    return Route;
}());
exports.Route = Route;
...
function trimSlashes(str) {
    return str.slice(str[0] === '/' ? 1 : 0, str[str.length - 1] === '/' ? -1 : str.length);
}
...
function parseUrlPath(url) {
    ...
    var _a = beginning.split('?'), path = _a[0], query = _a[1];
...
function parseQuery(query) {
    ...
    for (var _i = 0, pairs_1 = pairs; _i < pairs_1.length; _i++) {
        var pair = pairs_1[_i];
        var _a = pair.split('=').map(decodeURIComponent), name_1 = _a[0], value = _a[1];
        if (resultObj[name_1]) {
            if (!Array.isArray(resultObj[name_1])) {
                resultObj[name_1] = [resultObj[name_1]];
            }
            resultObj[name_1].push(value);
            continue;
        }
        resultObj[name_1] = value;
    }
    return resultObj;
}
exports.parseQuery = parseQuery;
function parseUrl(url) {
    var _a = parseUrlPath(url), path = _a.path, query = _a.query, hash = _a.hash;
    return { path: path, hash: hash, query: parseQuery(query) };
}
...
Object.defineProperty(exports, "__esModule", { value: true });
function handleHistory(router) {
    window.addEventListener('popstate', function (e) {
        var newState = e.state;
        router.go(newState.routeName, newState.data, false, false);
    });
    router.onTransition(function (type, oldCW, newCW, handleHistory) {
        if (!handleHistory) {
            return;
        }
        var url = router.build(newCW.route, newCW.data);
